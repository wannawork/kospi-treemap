...
- [수정] 필터링 조건 반영 동적 최대치 가이드라인(점선) 추가 및 IndexError 방지 로직 적용
"""
...
def build_treemap(df, date_str, kospi_idx, total_min, total_max, filtered_max):
    """
    filtered_max: 현재 필터 조건(섹터/종목 제외) 하에서의 2년 내 최대 시총
    """
    if df.empty:
        fig = go.Figure()
    ...
    # 박스 크기 비율 계산 (현재 필터링된 최대치 기준, 50% 하한선)
    if filtered_max > 0:
...
# ═══════════════════════════════════════
# MAIN (수정됨: IndexError 방지 및 필터 로직)
# ═══════════════════════════════════════
def main():
    check_password()
    ...
    # 시작 날짜 기준 필터링
    chart_start_val = st.session_state.get("chart_start", None)
    if chart_start_val:
        available_dates = [d for d in available_dates if d >= chart_start_val]
        df_history = df_history[df_history["Date"].dt.date >= chart_start_val].copy()

    if not available_dates:
        st.warning("선택한 기간에 데이터가 없습니다."); return

    # [FIX] IndexError 방지: 인덱스 초기화 및 범위 초과 시 자동 조정
    ...
    with st.sidebar:
        st.header("🔍 필터 설정")
        if source in ("sheets", "sheets_empty"): st.success("☁️ Google Sheets 연동 중")
        else: st.info("💾 로컬 CSV 모드")
        ...
        chart_start = st.date_input("시작 날짜", value=max(min_date, datetime(2026, 1, 1).date()), min_value=min_date, max_value=max_date, key="chart_start")
